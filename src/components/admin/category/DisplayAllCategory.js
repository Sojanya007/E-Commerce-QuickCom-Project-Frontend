...
export default function DisplayAllCategory() {
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={6} className={classes.center}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div className={classes.errorMessageStyle}>{errorMessages?.categoryIcon != null ? errorMessages?.categoryIcon : <span></span>}</div>
          </div>

        </Grid>
        <Grid item xs={6} className={classes.center}>
          <Avatar src={categoryIcon.fileName} style={{ width: 70, height: 70 }} variant='square' />
        </Grid>
  ...
  const handleEditData = async () => {
    ...
    if (err == false) {
      ...
      var body = {
        'categoryname': categoryName, 'updated_at': currentDate(),
